...
const UserReport: React.FC<Props> = memo(({
  ...
  const confirm = () => {
    ...
    addTitle(worksheet, t('menu.userReport') + ' ' + groups.filter(i => i.id == group)[0].name + ' ' + machines.filter(i => i.id == machine)[0].name, ((users || []).filter((item: any) => item.id == Number(user)))[0]['name'] + ' - ' + dayjs(period[0]).format('MMMM YYYY'))
  ...
  return (
    <div ref={div} className='wrapper'>
      <div style={{ display: 'inline-flex', width: '100%', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <Button disabled={loading} shape="circle" icon={<SaveOutlined style={{ fontSize: '130%' }} />} size="large" type="primary" style={{ margin: 10 }} onClick={confirm} ></Button>
      </div>
      <Table
        columns={columns}
        dataSource={data}
